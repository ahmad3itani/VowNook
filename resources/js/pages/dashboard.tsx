...
import { Armchair, Briefcase, CalendarClock, CalendarHeart, CheckCircle2, ListChecks, Users, Wallet } from 'lucide-react';
            ...
            <div className="flex flex-col gap-6 p-4">
                ...
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    <StatCard title="Guests" icon={Users} href="/guests">
                        ...
                        <div className="mt-3">
                            <Bar value={replied} total={guests.total} className="bg-rose-400" />
                        </div>
                    </StatCard>

                    <StatCard title="Budget" icon={Wallet} href="/budget">
                        <div className="text-2xl font-bold">{currency.format(budget.estimated)}</div>
                        <p className="text-xs text-muted-foreground">
                            {currency.format(budget.paid)} paid of {currency.format(budget.actual)} actual
                        </p>
                        <div className="mt-3">
                            <Bar value={budget.paid} total={budget.actual} className="bg-emerald-400" />
                        </div>
                    </StatCard>
                    ...
                    <StatCard title="Tasks" icon={ListChecks} href="/checklist">
                        <div className="text-2xl font-bold">
                            {tasks.completed}
                            <span className="text-base font-normal text-muted-foreground">/{tasks.total}</span>
                        </div>
                        <p className="text-xs text-muted-foreground">
                        ...
                        <div className="mt-3">
                            <Bar value={tasks.completed} total={tasks.total} className="bg-indigo-400" />
                        </div>
                    </StatCard>

                    <StatCard title="Seating" icon={Armchair} href="/seating">
                        <div className="text-2xl font-bold">{counts.seated}</div>
                        <p className="text-xs text-muted-foreground">
                            seated across {counts.tables} {counts.tables === 1 ? 'table' : 'tables'}
                        </p>
                        <div className="mt-3">
                            <Bar value={counts.seated} total={guests.total} className="bg-amber-400" />
                        </div>
                    </StatCard>
                </div>
                ...
                <div className="grid gap-4 lg:grid-cols-3">
                    <Card className="lg:col-span-2">
                        ...
                        <CardContent className="grid grid-cols-2 gap-4 sm:grid-cols-4">
                            {[
                                { label: 'Attending', value: guests.attending, tone: 'text-emerald-500' },
                                { label: 'Maybe', value: guests.maybe, tone: 'text-amber-500' },
                                { label: 'Declined', value: guests.declined, tone: 'text-rose-500' },
                                { label: 'Pending', value: guests.pending, tone: 'text-muted-foreground' },
                            ].map((s) => (
                    ...
                    <Card>
                        ...
                        <CardContent className="flex flex-col gap-3 text-sm">
                            <Link href="/vendors" className="flex items-center justify-between hover:text-primary">
                                <span className="flex items-center gap-2 text-muted-foreground">
                                    <Briefcase className="size-4" /> Vendors
                                </span>
                                <span className="font-semibold">{counts.vendors}</span>
                            </Link>
                            <Link href="/timeline" className="flex items-center justify-between hover:text-primary">
                                <span className="flex items-center gap-2 text-muted-foreground">
                                    <CalendarClock className="size-4" /> Timeline events
                                </span>
                                <span className="font-semibold">{counts.events}</span>
                            </Link>
                            <Link href="/checklist" className="flex items-center justify-between hover:text-primary">
                                <span className="flex items-center gap-2 text-muted-foreground">
                                    <CheckCircle2 className="size-4" /> Tasks done
                                </span>
