            ...
            <section className="px-5 pt-14 pb-10 md:px-12 md:pt-20">
                <div className="mx-auto max-w-[1480px]">
                    <nav className="mb-5 flex flex-wrap items-center gap-1.5 text-[12px] text-[#4b5850]">
                        ...
                        <span className="text-[#0f1c17]">{category.noun}</span>
                    </nav>
                    <p className="mb-3 text-[11px] tracking-[0.3em] text-[#1f5142] uppercase">Ontario</p>
                    <h1 className={`${fraunces} max-w-4xl text-4xl leading-[1.05] font-light sm:text-5xl md:text-6xl`}>
                        {category.noun} in <em className="text-[#1f5142]">Ontario</em>
                    </h1>
            ...
            {cost && (
                <section className="px-5 pb-10 md:px-12">
                    <div className="mx-auto max-w-[760px] rounded-2xl border border-[#0f1c17]/12 bg-white p-6 md:p-7">
                        <p className="text-[11px] tracking-[0.28em] text-[#1f5142] uppercase">Typical cost in Ontario</p>
                    ...
                    <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
                        {cities.map((c) => (
                            <Link
                                key={c.slug}
                                href={c.url}
                                className="group flex items-center justify-between rounded-xl border border-[#0f1c17]/12 bg-white px-4 py-3.5 transition-colors hover:border-[#1f5142]"
                            >
                                <span className="flex items-center gap-2 text-sm font-medium">
                                    <MapPin className="size-3.5 text-[#1f5142]" />
                                    {c.name}
                                </span>
                                {c.count > 0 && <span className="text-xs text-[#4b5850]">{c.count}</span>}
                            </Link>
                        ))}
                    </div>
                </div>
